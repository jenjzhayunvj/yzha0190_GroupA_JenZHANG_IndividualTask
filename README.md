...
Once the audio begins, the mushrooms become reactive and start moving with the music.
...
In my prototype, the mushrooms move based on the music. I mainly animate three things:

- Mushrooms grow and shrink gently with the volume. The whole mushroom scales up and down like breathing when the music gets loud or soft.

- Stems wiggle more when the drum beats hit. Low sounds make the stems bend side to side, so it looks like the mushrooms are dancing to the beat.

- The background becomes brighter when the music gets sharper. Higher sounds make the background glow a bit more, so the whole scene feels more alive.
...
[Pueblo Mágico by Quincas Moreira](https://www.youtube.com/watch?v=RWRI6xUi7A0) has a gentle rhythm and clear layers, which makes the mushroom’s movements feel more organic. The bass drives the stem’s wobble, the mid-range affects the breathing motion, and the higher frequencies make the background colours shift.

## 2.4 Technical explanation

## 2.4.1 Changes Made to Group Code

I did not make major structural changes to the group code. The existing geometry system, dot patterns, and Voronoi stem textures remain intact. My changes mainly extend the behavior inside the update and drawing functions so each mushroom receives the audio values and animates in response.
...
This task helped me explore audio-reactive animation and also taught me how to collaborate through GitHub for the first time. I learned how to use amplitude and FFT data to make the mushrooms react to music, and how to add new behavior on top of our group’s code while keeping everything working well. It feels great to see the artwork come alive with sound, and I now understand the audio part much better than before.
